 ...
 * 用户Controller 访问地址:
 * 
 * http://localhost:8086/sys/newuser/listUser
 * http://localhost:8086/sys/newuser/findById/13
 * http://localhost:8086/sys/newuser/save?username=tanzxuhao
 * http://localhost:8086/sys/user/update?userId=5&username=小米
 * http://localhost:8086/sys/newuser/delete/1
 * 
 * @author tanzuhao
 *
 */
@RequestMapping("/sys/newuser")
@RestController
@Api(tags = { "APP端-用户管理(新)" }, description = "APP端-用户管理(新)")
public class UserNewController {
	...
	public Result findById(@ApiParam(required = true, value = "用户ID") @PathVariable Long userId) {
		NewUser user = userService.get(userId);
	...
	public Result update(@RequestBody UserParamBean userBean) {
		NewUser user = userService.get(userBean.getUserId());
